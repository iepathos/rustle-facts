...
async fn test_real_rustle_parse_output() {
    ...
    match result {
        Ok(report) => {
            ...
            assert!(report.total_hosts == 0 || report.total_hosts > 0);
            
            let output_str = String::from_utf8(output).unwrap();
            
            let enriched: EnrichedPlaybook = serde_json::from_str(&output_str).unwrap();
            
            assert_eq!(enriched.playbook.metadata.file_path, 
            ...
            assert_eq!(enriched.playbook.plays[0].tasks[4].when, 
